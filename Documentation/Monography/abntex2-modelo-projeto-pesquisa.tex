...
%Assinaturas
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\assinatura{\textbf{nome banca 1} \\ Instituto Federal Fluminense(IFF)}

\assinatura{\textbf{nome banca 2} \\ Instituto Federal Fluminense(IFF)}
...
Este trabalho apresenta o desenvolvimento de uma arquitetura computacional didática, baseada no RISC~I e adaptada para o padrão RISC-V, com o objetivo de facilitar o ensino de conceitos fundamentais de arquitetura de computadores. O projeto teve como propósito geral criar uma ferramenta educacional que integrasse um modelo de simulação funcional, um \textit{assembler} customizado e uma documentação detalhada, promovendo uma experiência de aprendizado prática e interativa. A implementação incluiu a criação de um modelo de simulação no Logisim Evolution e em SystemVerilog, permitindo a visualização e análise do funcionamento interno do processador. Foram realizadas melhorias significativas na arquitetura, como a remoção das janelas de registradores, a introdução de um novo sistema de interrupções e exceções, e a implementação de dispositivos de entrada e saída, como teclado e tela gráfica. A arquitetura foi otimizada com uma \textit{pipeline} revisada, reduzindo o número de ciclos por instrução, e com suporte a registradores de controle e status (CSRs), alinhando-se às especificações do RISC-V. Além disso, foi desenvolvido um \textit{assembler} customizado, que oferece suporte a pseudoinstruções, macros e variáveis, facilitando a criação de programas para a arquitetura. A integração com ferramentas de simulação permitiu a execução de programas de teste, como algoritmos de ordenação e interação gráfica, demonstrando a aplicabilidade prática do projeto. A arquitetura também foi implementada em FPGA, comprovando sua utilidade em hardware real e tornando o trabalho mais completo. Os resultados destacam a relevância didática da arquitetura desenvolvida, que combina simplicidade e modernidade, proporcionando uma base sólida para o ensino de arquitetura de computadores e servindo como um recurso valioso para estudantes e educadores.
...
This work presents the development of a didactic computer architecture based on RISC~I and adapted to the RISC-V standard, aiming to facilitate the teaching of fundamental computer architecture concepts. The project’s overall goal was to create an educational tool that integrates a functional simulation model, a custom assembler, and detailed documentation, fostering a practical and interactive learning experience. The implementation included building a simulation model in Logisim Evolution and in SystemVerilog, enabling visualization and analysis of the processor’s internal operations. Significant enhancements were made to the architecture, such as the removal of register windows, the introduction of a new interrupt and exception system, and the implementation of input/output devices like a keyboard and graphical display. The architecture was optimized with a revised pipeline, reducing the number of cycles per instruction, and with support for control and status registers (CSRs), aligning with the RISC-V specifications. Additionally, a custom assembler was developed, offering support for pseudoinstructions, macros, and variables, thus simplifying program development for the architecture. Integration with simulation tools enabled the execution of test programs—including sorting algorithms and graphical interactions—demonstrating the project’s practical applicability. The architecture was also implemented on FPGA, confirming its real-world utility and making the work more complete. The results underscore the didactic relevance of the proposed architecture, which combines simplicity and modernity to provide a solid foundation for teaching computer architecture and to serve as a valuable resource for students and educators.
...
\begin{enumerate}[label=\textbf{O\arabic*}]
    ...
    \item Executar uma bibliometria sobre uma arquitetura previamente escolhida em comparação a outras. \label{ob:bibliometria}
...
\begin{itemize}
    ...
    \item \textbf{Pipeline de cinco estágios} (\ref{recursos_pipe5}) — semelhante à utilizada no MIPS, requer separação entre os barramentos de dados e instruções para evitar conflitos de acesso à memória, geralmente resolvidos com cache L1, que não foi implementado;
...
A bibliometria, referente ao Objetivo \ref{ob:bibliometria}, embora represente apenas uma das etapas na pesquisa por arquiteturas relevantes, introduz uma técnica inovadora inspirada em estudos bibliométricos anteriores. Essa abordagem tem potencial para se consolidar como uma ferramenta eficiente na realização de análises rápidas sobre a relevância de temas ao longo do tempo.
...
Embora o projeto tenha alcançado seus principais objetivos, algumas limitações foram identificadas, como a ausência de recursos mais avançados, como cache, operações com ponto flutuante e multithreading. Esses aspectos podem ser explorados em trabalhos futuros, ampliando ainda mais o potencial didático e funcional da arquitetura.

Em suma, este trabalho contribui significativamente para o ensino de arquitetura de computadores, oferecendo uma plataforma prática e acessível para o aprendizado. A arquitetura DRISC-V, com sua simplicidade e flexibilidade, representa um avanço na criação de ferramentas educacionais na área, alinhando-se às necessidades de ensino e pesquisa em computação.
